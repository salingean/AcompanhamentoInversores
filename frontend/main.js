...
const API_URL = 'http://localhost:3000/api/inversores';
...
function getBaseOptions() {
    return JSON.parse(JSON.stringify(commonOptions));
}
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            if (expandedChartInstance && expandModal.dataset.currentId === id) {
                const base = getBaseOptions();
                if (forceZoom) {
                    expandedChartInstance.updateOptions({ xaxis: { ...base.xaxis } }, false, false, false);
                }
...
function renderDashboard(dataToRender) {
    ...
    dataToRender.forEach(inversor => { try {
        ...
        if (card) {
            ...
            filtersDiv.append(dateSelectorContainer, sInput, spanA, eInput);
...
function openExpandedModal(id, name) {
    ...
    const expandedOptions = {
        ...
        yaxis: generateYAxis(chartInstances[id].w.config.series),
